...
async function dispatchAiPrompt(promptText, apiKey, provider = 'auto', model = '', customEndpoint = '') {
  ...
  const callProvider = async (pTarget, overrideModel = null) => {
    ...
    if (pTarget === 'custom' && customEndpoint) {
      ...
      activeModel = targetModel || 'llama-3.3-70b-versatile';
  ...
  let primaryTarget = (provider && provider !== 'auto') ? provider : detectedProvider;
  if (detectedProvider !== 'gemini' && primaryTarget !== detectedProvider) {
    logger.warn(`API key prefix indicates provider "${detectedProvider}". Overriding configured provider "${primaryTarget}".`);
    primaryTarget = detectedProvider;
  }
  ...
  try {
    ...
    const isQuotaError = err.message && (err.message.includes('429') || err.message.includes('Quota exceeded') || err.message.includes('RESOURCE_EXHAUSTED') || err.message.includes('out of tokens') || err.message.includes('Rate limit'));
    if (isQuotaError && primaryTarget === 'groq') {
      const fallbackGroqModel = (model && model !== 'llama-3.1-8b-instant') ? 'llama-3.1-8b-instant' : 'llama-3.3-70b-versatile';
      logger.warn(`Groq rate limit exceeded for "${model || 'default'}". Falling back to high-quota model "${fallbackGroqModel}"...`);
      try {
        return await callProvider('groq', fallbackGroqModel);
      } catch (_) {}
    }
