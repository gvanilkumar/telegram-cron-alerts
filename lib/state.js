...
const stateManager = {
  ...
  writeHistoryLog(logEntry) {
    try {
      ...
      // Async background call to Google Sheets if enabled
      if (config.settings.googleSheetLoggingEnabled && config.googleSheetId && config.googleServiceAccountKey) {
        ...
        appendSheetLog(config.googleSheetId, config.googleServiceAccountKey, logEntry)
          .catch(err => logger.error('Background Google Sheets logging failed', err));
      }
    } catch (err) {
