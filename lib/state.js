...
const stateManager = {
  ...
  async writeHistoryLog(logEntry) {
    try {
      ...
      // Async call to Google Sheets if credentials exist
      const sheetsLoggingEnabled = (config.settings.googleSheetLoggingEnabled !== false) && Boolean(config.googleSheetId) && Boolean(config.googleServiceAccountKey);
      if (sheetsLoggingEnabled) {
        try {
