...
async function scrapeWebpage(url) {
  ...
  try {
    ...
    if (jinaResponse.ok) {
      ...
      if (text && text.trim().length > 100) {
        ...
        return text.trim().substring(0, 2500); // Truncate to fit all LLM context limits
      }
    }
  ...
  try {
    ...
    // Limit to 2500 characters for fallback to fit small local context configurations
