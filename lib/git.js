...
async function gitSync() {
  ...
  try {
    // 1. Stage all config files (tasks, settings, and state)
    await runCmd('git add tasks.json settings.json state.json');
    ...
    try {
      ...
      await runCmd('git -c core.editor=true pull --rebase -X theirs origin HEAD');
    } finally {
      if (stashed) {
        try {
